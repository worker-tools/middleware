...
test('basics/method', async () => {
  const { method } = await withBasics()({ request, effects: [] })
  expect(method).toBe('POST')
})

test('basics/url', async () => {
  const { url } = await withBasics()({ request, effects: [] })
  expect(url).toBeInstanceOf(URL)
})

test('basics/headers', async () => {
  const { headers } = await withBasics()({ request, effects: [] })
  expect(headers).toBeInstanceOf(Headers)
  expect(headers.get('user-agent')).toBe('Mozilla/5.0 (Windows; U; MSIE 9.0; Windows NT 9.0; en-US);')
})

test('basics/pathname', async () => {
  const { pathname } = await withBasics()({ request, effects: [] })
  expect(pathname).toBe('/item/detail')
})

test('basics/search', async () => {
  const { searchParams, query } = await withBasics()({ request, effects: [] })
...
test('basics/userAgent', async () => {
  const { userAgent } = await withBasics()({ request, effects: [] })
